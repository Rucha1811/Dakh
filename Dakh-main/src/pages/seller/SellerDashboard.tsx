...
import {
  ...
  AlertTriangle, CheckCircle, Info, Plus, MapPin, Search,
} from 'lucide-react';
...
export default function SellerDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Gujarat Dak Ghar Niryat Kendra Map & Hubs */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-[var(--color-primary)] dark:text-white flex items-center gap-2">
              <MapPin className="w-5 h-5 text-[var(--color-brand-red)]" />
              Gujarat DNK Export Post Office Hubs
            </h3>
            <p className="text-xs text-gray-500 dark:text-slate-400">
              Direct postal export counters across Ahmedabad, Surat, Kutch/Bhuj, Vadodara, Rajkot, and all Gujarat districts
            </p>
          </div>
          <Link
            to="/seller/dnk"
            className="text-xs font-semibold text-[var(--color-brand-red)] hover:underline flex items-center gap-1 bg-red-50 dark:bg-red-950/40 px-3 py-1.5 rounded-lg border border-red-200 dark:border-red-800"
          >
            Full Screen Locator <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
        <GujaratMap
          dnks={DNKS}
