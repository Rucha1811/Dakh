...
export default function Profile() {
  ...
  const seller = useMemo(() => (isSeller ? getSellerForUser(user) : undefined), [isSeller, user]);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-800">{t('profile.businessInfo', language)}</h3>
          <button
            onClick={() => setEditing(!editing)}
            className="text-sm text-[var(--color-primary)] font-medium hover:underline"
          >
            {editing ? t('profile.cancel', language) : t('profile.edit', language)}
          </button>
        </div>
        ...
        <div className="space-y-4">
          {editing ? (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('settings.fullName', language)}</label>
                <input
                  type="text"
                  value={name}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('settings.phone', language)}</label>
                <input
                  type="text"
                  value={phone}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('settings.location', language)}</label>
                <input
                  type="text"
                  value={location}
            ...
            <>
              <InfoRow icon={User} label={t('settings.fullName', language)} value={user.name} />
              <InfoRow icon={Mail} label={t('common.email', language)} value={user.email} />
              <InfoRow icon={Phone} label={t('settings.phone', language)} value={user.phone} />
              <InfoRow icon={MapPin} label={t('settings.location', language)} value={user.location} />
              {seller && (
                <>
                  <InfoRow icon={Shield} label={t('profile.businessInfo', language)} value={seller.businessName} />
                  <InfoRow icon={Shield} label={t('common.category', language)} value={seller.businessType} />
                  <InfoRow icon={MapPin} label={t('settings.location', language)} value={`${seller.address}, ${seller.district}, ${seller.state} - ${seller.pinCode}`} />
                </>
              )}
            </>
          )}
