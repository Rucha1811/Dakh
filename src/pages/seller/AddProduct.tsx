...
export default function AddProduct() {
  ...
  const handleSubmit = () => {
    ...
    const productData = {
      ...
      images: isEditing ? editProduct!.images : [],
        ...
        <div className="space-y-5">
          ...
          {step === 3 && (
            <div className="space-y-4 animate-fade-in">
              ...
              {imageCount > 0 && (
                <div className="grid grid-cols-3 gap-3">
                  {Array.from({ length: imageCount }).map((_, i) => (
                    <div key={i} className="aspect-square rounded-lg bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center relative">
                      <Upload className="h-6 w-6 text-gray-400" />
                      <button
                        onClick={() => setImageCount((c) => c - 1)}
                        className="absolute -top-2 -right-2 w-5 h-5 bg-[var(--color-brand-red)] text-white rounded-full flex items-center justify-center text-xs"
                      >
                        ×
                      </button>
                    </div>
              ...
              <button
                onClick={() => setImageCount((c) => c + 1)}
                className="w-full border-2 border-dashed border-gray-300 rounded-xl p-10 text-center hover:bg-gray-50 transition-colors cursor-pointer"
              >
